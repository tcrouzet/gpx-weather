...
function tripDetailCard(day){{const samples=conditionsAlongDay(day);return `<div class="trip-detail-card"><h3>${{day.label}} · ${{day.morningTown.name}} → ${{day.eveningTown.name}}</h3><div class="journey-day-stats">${{day.distance}} km · D+ ${{day.gain}} m · ${{day.speed.toFixed(1)}} km/h</div>${{journeyRouteMap(day)}}${{journeyChart('Température et ressenti (°C)',samples,[{{label:'Température',color:'#e34a33',value:s=>s.conditions?.temperature??NaN}},{{label:'Ressenti',color:'#f39c12',value:s=>s.conditions?.apparent_temperature??NaN}}])}}${{journeyChart('Précipitations et nuages',samples,[{{label:'Probabilité (%)',color:'#2474b5',value:s=>s.conditions?.rain_probability??0}},{{label:'Nuages (%)',color:'#7d8795',value:s=>s.conditions?.cloud_cover??0}},{{label:'Précipitations (mm ×10)',color:'#20a9c9',value:s=>(s.conditions?.precipitation??0)*10}}])}}${{journeyChart('Vent et rafales (km/h)',samples,[{{label:'Vent',color:'#24965f',value:s=>s.conditions?.wind??0}},{{label:'Rafales',color:'#c9364f',value:s=>s.conditions?.gusts??0}}])}}${{journeyChart('Élévation (m)',samples,[{{label:'Altitude',color:'#82552c',fill:'#b887563f',value:s=>s.elevation}}])}}</div>`}}
