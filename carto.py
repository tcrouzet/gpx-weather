...
function elevationAlongDay(day,count=Math.max(2,Math.round(day.rideHours*4)+1)){{return Array.from({{length:count}},(_,index)=>{{const rideRatio=index/Math.max(1,count-1),clock=day.departureHour+day.rideHours*rideRatio,effort=day.startEffort+day.dayEffort*rideRatio,distance=distanceAtEffort(effort);return {{clock,distance,elevation:elevationAtDistance(distance)}}}})}}
function journeyChart(title,samples,series){{const width=400,height=150,pad=28,legend=`<div class="journey-series-legend">${{series.map(item=>`<span style="--series-color:${{item.color}}">${{item.label}}</span>`).join('')}}</div>`,values=series.flatMap(item=>samples.map(sample=>Number(item.value(sample))).filter(Number.isFinite));if(!values.length)return `<div class="journey-chart"><h4>${{title}}</h4><svg class="journey-plot" viewBox="0 0 400 150"><text x="200" y="78" fill="#888" text-anchor="middle">Indisponible</text></svg>${{legend}}</div>`;const minimum=Math.min(...values),maximum=Math.max(...values),span=Math.max(1,maximum-minimum),x=index=>pad+index*(width-pad*2)/(samples.length-1),y=value=>height-pad-(value-minimum)*(height-pad*2)/span;const grid=[0,.5,1].map(ratio=>`<line x1="${{pad}}" y1="${{pad+ratio*(height-pad*2)}}" x2="${{width-pad}}" y2="${{pad+ratio*(height-pad*2)}}" stroke="#c9ced8"/>`).join(''),lines=series.map(item=>{{const coordinates=samples.map((sample,index)=>[index,Number(item.value(sample))]).filter(point=>Number.isFinite(point[1])),points=coordinates.map(point=>`${{x(point[0])}},${{y(point[1])}}`).join(' ');if(!points)return '';const area=item.fill?`<polygon points="${{x(coordinates[0][0])}},${{height-pad}} ${{points}} ${{x(coordinates.at(-1)[0])}},${{height-pad}}" fill="${{item.fill}}"/>`:'';return `${{area}}<polyline points="${{points}}" fill="none" stroke="${{item.color}}" stroke-width="2.5" stroke-linejoin="round"/>`}}).join(''),labels=samples.map((sample,index)=>{{const hour=Math.round(sample.clock);return Math.abs(sample.clock-hour)<.05&&hour%2===0?`<text x="${{x(index)}}" y="14" fill="#68708c" font-size="9" text-anchor="middle">${{String(hour).padStart(2,'0')}}h</text>`:''}}).join('');return `<div class="journey-chart"><h4>${{title}}</h4><svg class="journey-plot" viewBox="0 0 ${{width}} ${{height}}" preserveAspectRatio="none">${{grid}}${{lines}}${{labels}}<text x="4" y="${{pad+4}}" fill="#68708c" font-size="9">${{Math.round(maximum)}}</text><text x="4" y="${{height-pad+4}}" fill="#68708c" font-size="9">${{Math.round(minimum)}}</text></svg>${{legend}}</div>`}}
...
function tripDetailCard(day){{const samples=conditionsAlongDay(day),elevationSamples=elevationAlongDay(day);return `<div class="trip-detail-card"><h3>${{day.label}} · ${{day.morningTown.name}} → ${{day.eveningTown.name}}</h3><div class="journey-day-stats">${{day.distance}} km · D+ ${{day.gain}} m · ${{day.speed.toFixed(1)}} km/h</div>${{journeyRouteMap(day)}}${{journeyChart('Température et ressenti (°C)',samples,[{{label:'Température',color:'#e34a33',value:s=>s.conditions?.temperature??NaN}},{{label:'Ressenti',color:'#f39c12',value:s=>s.conditions?.apparent_temperature??NaN}}])}}${{journeyChart('Précipitations et nuages',samples,[{{label:'Probabilité (%)',color:'#2474b5',value:s=>s.conditions?.rain_probability??0}},{{label:'Nuages (%)',color:'#7d8795',value:s=>s.conditions?.cloud_cover??0}},{{label:'Précipitations (mm ×10)',color:'#20a9c9',value:s=>(s.conditions?.precipitation??0)*10}}])}}${{journeyChart('Vent et rafales (km/h)',samples,[{{label:'Vent',color:'#24965f',fill:'#24965f42',value:s=>s.conditions?.wind??0}},{{label:'Rafales',color:'#c9364f',fill:'#c9364f2e',value:s=>s.conditions?.gusts??0}}])}}${{journeyChart('Élévation (m)',elevationSamples,[{{label:'Altitude',color:'#82552c',fill:'#b887563f',value:s=>s.elevation}}])}}</div>`}}
