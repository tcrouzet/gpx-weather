...
.journey-chart>svg.journey-plot{{display:block;width:100%;height:150px;background:#f7f8fb;border-radius:5px}}.journey-chart.wide,.journey-map{{grid-column:1/-1}}.journey-series-legend{{display:flex;flex-wrap:wrap;gap:4px 12px;min-height:18px;padding:4px 2px 0;color:#4c5875;font-size:10px;font-weight:700}}.journey-series-legend span::before{{content:'';display:inline-block;width:12px;height:4px;margin-right:4px;border-radius:2px;background:var(--series-color);vertical-align:middle}}.journey-chart-meta{{grid-column:1/-1;text-align:center;font-size:11px;color:#68708c}}.journey-mini-map{{width:100%;height:300px;border-radius:5px;background:#dce8df}}.journey-wind-marker{{background:transparent;border:0;color:#fff;font-size:24px;font-weight:900;text-shadow:-1px -1px 1px #17234d,1px 1px 1px #17234d}}.journey-wind-marker span{{display:block;line-height:1;text-align:center}}.journey-wind-marker.wind-strong{{font-size:31px}}.journey-wind-marker.wind-danger{{font-size:34px;color:#e31b23;text-shadow:-1px -1px 1px #fff,1px 1px 1px #fff,0 0 3px #17234d}}.journey-town-name{{width:auto!important;height:auto!important;background:#fff;border:1px solid #18295c;border-radius:4px;color:#17234d;font-size:10px;font-weight:850;line-height:16px;padding:0 4px;box-shadow:0 1px 3px #0005;white-space:nowrap;cursor:pointer}}.journey-town-name:hover{{background:#18295c;color:#fff}}.journey-legend{{font-size:10px;color:#68708c}}
...
.metric{{min-height:50px;padding:5px 7px}}.trip-detail-card{{grid-template-columns:1fr}}.trip-detail-card h3,.journey-chart.wide,.journey-map,.journey-chart-meta{{grid-column:1}}.journey-mini-map{{height:250px}}.journey-chart>svg.journey-plot{{height:135px}}.metric-value{{font-size:14px}}.trip-planner{{padding:9px 7px}}.planner-form{{grid-template-columns:1fr 1fr;gap:7px}}.planner-form label:first-child{{grid-column:1/-1}}.planner-form input{{height:42px;min-height:42px;padding:7px}}.trip-day{{padding:7px}}.trip-metrics{{gap:4px}}.trip-stop{{padding:6px 3px;min-height:100px}}.trip-stop-value{{font-size:16px}}}}
...
function tripDetailCard(day){{const samples=conditionsAlongDay(day);return `<div class="trip-detail-card"><h3>${{day.label}} · ${{day.morningTown.name}} → ${{day.eveningTown.name}}</h3>${{journeyRouteMap(day)}}${{journeyChart('Température et ressenti (°C)',samples,[{{label:'Température',color:'#3182ce',value:s=>s.conditions?.temperature??NaN}},{{label:'Ressenti',color:'#f6a800',value:s=>s.conditions?.apparent_temperature??NaN}}])}}${{journeyChart('Précipitations et nuages',samples,[{{label:'Probabilité (%)',color:'#4da3ff',value:s=>s.conditions?.rain_probability??0}},{{label:'Nuages (%)',color:'#9aa0aa',value:s=>s.conditions?.cloud_cover??0}},{{label:'Précipitations (mm ×10)',color:'#f6a800',value:s=>(s.conditions?.precipitation??0)*10}}])}}${{journeyChart('Vent et rafales (km/h)',samples,[{{label:'Vent',color:'#3182ce',value:s=>s.conditions?.wind??0}},{{label:'Rafales',color:'#f6a800',value:s=>s.conditions?.gusts??0}}])}}${{journeyChart('Élévation (m)',samples,[{{label:'Altitude',color:'#f6a800',value:s=>s.elevation}}])}}<div class="journey-chart-meta">${{day.distance}} km · D+ ${{day.gain}} m · ${{day.speed.toFixed(1)}} km/h<br><span class="journey-legend">Vert : départ · rouge : arrivée</span></div></div>`}}
function initJourneyMaps(){{currentTripPlan.forEach(day=>{{const element=document.querySelector(`#journey-map-${{day.index}}`);if(!element)return;const start=Math.max(0,Math.round(day.startDistance/data.route_distance_km*(data.route.length-1))),end=Math.min(data.route.length-1,Math.round(day.endDistance/data.route_distance_km*(data.route.length-1))),points=data.route.slice(start,Math.max(start+2,end+1)),mini=L.map(element,{{zoomControl:true,attributionControl:false,dragging:true,scrollWheelZoom:true,doubleClickZoom:true,boxZoom:true,keyboard:true,touchZoom:true}});L.tileLayer('https://tile.openstreetmap.org/{{z}}/{{x}}/{{y}}.png',{{maxZoom:19}}).addTo(mini);const casing=L.polyline(points,{{color:'#fff',weight:9,opacity:.95}}).addTo(mini),line=L.polyline(points,{{color:'#d62728',weight:5,opacity:1}}).addTo(mini);mini.fitBounds(line.getBounds(),{{padding:[22,22]}});L.circleMarker(points[0],{{radius:6,color:'#111',weight:2,fillColor:'#28c840',fillOpacity:1}}).addTo(mini);L.circleMarker(points.at(-1),{{radius:6,color:'#111',weight:2,fillColor:'#ff3b30',fillOpacity:1}}).addTo(mini);data.planner_towns.filter(town=>town.distance_km>=day.startDistance-1&&town.distance_km<=day.endDistance+1).forEach(town=>{{const icon=L.divIcon({{className:'journey-town-name',html:town.name,iconSize:null,iconAnchor:[0,8]}}),marker=L.marker([town.lat,town.lon],{{icon,zIndexOffset:1000}}).addTo(mini);marker.on('click',()=>focusPlannerTown(town,day.dateKey,12))}});conditionsAlongDay(day,11).forEach(sample=>{{const routeIndex=Math.min(data.route.length-1,Math.max(0,Math.round(sample.distance/data.route_distance_km*(data.route.length-1)))),angle=((sample.conditions?.wind_degrees??0)+180)%360,speed=Number(sample.conditions?.wind)||0,windClass=speed>45?' wind-danger':speed>25?' wind-strong':'',size=speed>45?36:speed>25?32:24,icon=L.divIcon({{className:`journey-wind-marker${{windClass}}`,html:`<span style="transform:rotate(${{angle}}deg)">↑</span>`,iconSize:[size,size],iconAnchor:[size/2,size/2]}}),marker=L.marker(data.route[routeIndex],{{icon,interactive:true,zIndexOffset:2000}}).addTo(mini);marker.bindTooltip(sample.conditions?`${{sample.conditions.wind}} km/h`:'Vent indisponible',{{direction:'top',offset:[0,-10]}})}})}})}}
